...
String ac=request.getParameter("ta");
double acnt=Double.parseDouble(ac);
...
double next=0;
while(rs.next()){
	prev=rs.getDouble("amount");
}
ResultSet rp=st.executeQuery("select amount from users where acount='"+acnt+"';");
while(rp.next()){
	next=rp.getDouble("amount");
}
prev=prev-amount;
next=next+amount;
int i=st.executeUpdate("update users set amount='"+prev+"' where acount='"+account+"'"); 
int j=st.executeUpdate("update users set amount='"+next+"' where acount='"+acnt+"'"); 
%>
...
<table  border="5"; style=" margin-top: 130px ">
	<tr>
		...
		<td style="height: 1;width: 500;">
			Transfer Sucessful
			Your new bal is<%out.println(prev); %>
			</td>
	</tr>
</table>
</body>
